...
function setup() {
    ...
    sword = new Blade(color("#9cad98")); // change sword color
    frameRate(60); //60 FPS
...
function draw() {
    background(206,22,83); //background color of map
...
// fruit cutting
	...
	for (var i = fruit.length - 1; i >= 0; i--) {
		...
		if (!fruit[i].visible) { // if fruit is no longer on the screen
			...
			points += (sword.checkForSlice(fruit[i])) ? 10 : 0; // if fruit is cut, add 1 
		}
